"""Handle table: integer ids <-> opaque C pointers, WITH OWNERSHIP.

Ownership is the cJSON lesson. cJSON_GetObjectItem returns a pointer INTO the
tree you passed in; it is owned by the parent. If the client could free it, the
parent's later delete would double-free. So every handle records whether the
caller owns it (may free) or merely borrows it (must not free), and the table
REFUSES to pop a borrowed handle.
"""
import itertools
import threading
